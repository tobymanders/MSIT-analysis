...
parfor prm = 1:nPerms
    ...
    [nullDist(:,:,prm),nullPhase(:,:,prm),~,~,~,t,f] = cohgramcpt(continuousData(:,randIdxCont),pointProcessData(randIdxPt), movingWin, params);
...
%% [20160418] - This works, but is highly inefficient. 
% display('running shuffle test to determine null distribution for spike-field coherence')
% for tt = 1:nTrials
%     % updating user each trial.
%     display(sprintf('permuting spike trials %d times for LFP trial %d out of %d',nPerms,tt,nTrials))
%     
%     for prm = 1:nPerms
%         
%         % updating user every 50 permutations.
...
%         
%         % calculating coherence.
%         [nullDist(:,:,tt,prm),nullPhase(:,:,tt,prm),~,~,~,t,f] = cohgramcpt(continuousData(:,tt),pointProcessData(randIdxPt(prm)), movingWin, params);
%         
